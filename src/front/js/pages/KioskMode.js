...
const KioskMode = () => {
    ...
    return (
        <div style={{height:"100vh",background:"linear-gradient(135deg,#0a0e1a,#1a2040)",color:"#fff8e1",fontFamily:"system-ui",overflow:"hidden",display:"flex",flexDirection:"column"}}>
            {/* Header */}
            <div style={{padding:"1rem 2rem",background:"rgba(0,0,0,0.4)",display:"flex",justifyContent:"space-between",alignItems:"center",borderBottom:"1px solid rgba(255,255,255,0.1)"}}>
                ...
                <div style={{display:"flex",gap:"1rem",alignItems:"center"}}>
                    {cart.length>0 && step==="browse" && (
                        <button style={{background:"#2dce89",border:"none",borderRadius:"8px",padding:"0.5rem 1.5rem",color:"#fff8e1",fontWeight:600,cursor:"pointer",fontSize:"1rem"}}
                            onClick={()=>setStep("cart")}>
                            🛒 Cart ({cart.reduce((s,i)=>s+i.qty,0)}) — ${total.toFixed(2)}
                        </button>
                    )}
                    {step!=="browse" && <button style={{background:"rgba(255,255,255,0.1)",border:"1px solid rgba(255,255,255,0.2)",borderRadius:"8px",padding:"0.5rem 1rem",color:"#fff8e1",cursor:"pointer"}} onClick={resetKiosk}>✕ Start Over</button>}
                </div>
            </div>
            ...
            {step === "browse" && (
                <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
                    <div style={{padding:"1rem 2rem",display:"flex",gap:"1rem",alignItems:"center",background:"rgba(0,0,0,0.2)"}}>
                        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search products..." style={{flex:1,padding:"0.75rem 1rem",borderRadius:"8px",border:"1px solid rgba(255,255,255,0.2)",background:"rgba(255,255,255,0.05)",color:"#fff8e1",fontSize:"1rem"}} />
                        <div style={{display:"flex",gap:"0.5rem",flexWrap:"wrap"}}>
                            {CATEGORIES.map(c=><button key={c} onClick={()=>setCategory(c)} style={{padding:"0.5rem 1rem",borderRadius:"20px",border:"none",background:category===c?"#2dce89":"rgba(255,255,255,0.1)",color:"#fff8e1",cursor:"pointer",fontWeight:category===c?700:400}}>{c}</button>)}
                        </div>
                    </div>
                        ...
                        {filtered.map(p=>(
                            <div key={p.id} onClick={()=>addToCart(p)} style={{background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:"12px",padding:"1.25rem",cursor:"pointer",transition:"transform 0.15s,border-color 0.15s"}}
                                ...
                                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                                    ...
                                    <span style={{background:"#2dce89",borderRadius:"50%",width:"32px",height:"32px",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"1.2rem",color:"#fff8e1"}}>+</span>
                                </div>
                            </div>
                        ))}
                    </div>
            ...
            {step === "cart" && (
                <div style={{flex:1,display:"flex",flexDirection:"column",padding:"2rem",overflow:"auto"}}>
                    ...
                    {cart.map(item=>(
                        <div key={item.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"1rem",marginBottom:"0.75rem",background:"rgba(255,255,255,0.06)",borderRadius:"10px"}}>
                            ...
                            <div style={{display:"flex",alignItems:"center",gap:"1rem"}}>
                                <button onClick={()=>updateQty(item.id,item.qty-1)} style={{width:"36px",height:"36px",borderRadius:"50%",border:"1px solid rgba(255,255,255,0.2)",background:"transparent",color:"#fff8e1",fontSize:"1.2rem",cursor:"pointer"}}>-</button>
                                <span style={{fontWeight:700,fontSize:"1.1rem",minWidth:"24px",textAlign:"center"}}>{item.qty}</span>
                                <button onClick={()=>updateQty(item.id,item.qty+1)} style={{width:"36px",height:"36px",borderRadius:"50%",border:"none",background:"#2dce89",color:"#fff8e1",fontSize:"1.2rem",cursor:"pointer"}}>+</button>
                    ...
                    <div style={{display:"flex",gap:"1rem",marginTop:"1.5rem"}}>
                        <button onClick={()=>setStep("browse")} style={{flex:1,padding:"1rem",borderRadius:"10px",border:"1px solid rgba(255,255,255,0.2)",background:"transparent",color:"#fff8e1",fontSize:"1rem",cursor:"pointer"}}>← Continue Shopping</button>
                        <button onClick={()=>setStep("age-verify")} style={{flex:2,padding:"1rem",borderRadius:"10px",border:"none",background:"#2dce89",color:"#fff8e1",fontSize:"1.1rem",fontWeight:700,cursor:"pointer"}}>Proceed to Checkout →</button>
                    </div>
                </div>
            )}
                ...
                <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"2rem",textAlign:"center"}}>
                    ...
                    <div style={{background:"rgba(255,255,255,0.06)",borderRadius:"16px",padding:"2rem",width:"100%",maxWidth:"400px"}}>
                        <div style={{marginBottom:"1.5rem"}}>
                            ...
                            <input type="date" value={dob} onChange={e=>setDob(e.target.value)} style={{width:"100%",padding:"1rem",borderRadius:"8px",border:"1px solid rgba(255,255,255,0.2)",background:"rgba(255,255,255,0.05)",color:"#fff8e1",fontSize:"1.1rem"}} />
                        </div>
                        <button onClick={verifyAge} style={{width:"100%",padding:"1rem",borderRadius:"10px",border:"none",background:"#2dce89",color:"#fff8e1",fontSize:"1.1rem",fontWeight:700,cursor:"pointer",marginBottom:"1rem"}}>Verify Age</button>
                ...
                <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"2rem",textAlign:"center"}}>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"1rem",width:"100%",maxWidth:"500px",marginBottom:"2rem"}}>
                        {[{l:"💵 Cash",v:"cash"},{l:"💳 Card",v:"card"},{l:"🏦 Debit",v:"debit"},{l:"📱 Digital",v:"digital"}].map(pm=>(
                            <button key={pm.v} onClick={completeOrder} disabled={completing} style={{padding:"2rem",borderRadius:"12px",border:"1px solid rgba(255,255,255,0.2)",background:"rgba(255,255,255,0.06)",color:"#fff8e1",fontSize:"1.1rem",fontWeight:600,cursor:"pointer",transition:"all 0.2s"}}
            ...
            {step === "receipt" && (
                <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"2rem",textAlign:"center"}}>
                    ...
                    <button onClick={resetKiosk} style={{padding:"1rem 3rem",borderRadius:"10px",border:"none",background:"#2dce89",color:"#fff8e1",fontSize:"1.1rem",fontWeight:700,cursor:"pointer"}}>Start New Order</button>
                </div>
            )}
        </div>
    );
